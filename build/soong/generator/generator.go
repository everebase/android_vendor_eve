...
func init() {
	android.RegisterModuleType("eve_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = eveExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = eveExpandVariables(ctx, dep_file)
	...
	cmd := eveExpandVariables(ctx, String(g.properties.Cmd))
