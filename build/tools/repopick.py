...
if __name__ == '__main__':
    # Default to EVE Gerrit
    default_gerrit = 'https://review.eveos.com'
    ...
    for item in mergables:
        ...
        if args.gerrit == default_gerrit:
            ...
            if args.pull:
                cmd = ['git pull --no-edit eve', item['fetch'][method]['ref']]
            else:
                cmd = ['git fetch eve', item['fetch'][method]['ref']]
            if args.quiet:
                cmd.append('--quiet')
        ...
        if args.gerrit != default_gerrit or os.stat(FETCH_HEAD).st_size == 0:
            # If not using the default gerrit or eve failed, fetch from gerrit.
            if args.verbose:
                if args.gerrit == default_gerrit:
